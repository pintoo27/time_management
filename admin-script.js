class AdminDashboard {
  ...
  async loadTrainerSessions(trainerId) {
    ...
    try {
      const response = await fetch(`sessions.php?trainer_id=${trainerId}`)
  ...
  async loadTrainers() {
    try {
      const response = await fetch("trainers.php")
      const data = await response.json()
  ...
  //                       <div>🕒 ${session.session_time} - ${endTime} (${session.duration} min)</div>

...
//     return "session-missed";  // new orange class
//   }

//   if (this.isSessionTimeCompleted(session)) {
//     return "session-time-completed"; // default black/gray
//   }
...
//     return "session-card-completed";
//   } else if (session.status === "missed") {
//     return "session-card-missed";  // new class
//   }
...
//   async loadSessions() {
//     try {
//       const response = await fetch(`sessions.php?trainer_id=${this.currentTrainer.id}`)
//       const data = await response.json()
...
//   // ✅ Auto-fill trainer name as client (readonly)
//   document.getElementById("clientName").value = this.currentTrainer?.name || "Trainer"
...
// //////////////////////////////////////////////////////////////////////
//     let selectedDays = Array.from(document.querySelectorAll('input[name="weekdays"]:checked')).map((cb) =>
//   Number.parseInt(cb.value)
// )

// // If no days selected, default to startDate only (single session)
// if (selectedDays.length === 0) {
//   const startDate = new Date(formData.get("startDate"))
//   selectedDays = [startDate.getDay()]  // Use startDate's day to create one session
// }
...
//   alert("No valid sessions found for the selected input.")
//   submitBtn.style.display = "block"
//   spinner.style.display = "none"
//   return
// }
...
class TrainerDashboard {
  ...
  if (daySessions.length === 0) {
    ...
    const formatTimeFromDatetime = (datetime) => {
      if (!datetime) return "";
      const timePart = datetime.split(" ")[1]; // Get HH:MM:SS
      if (!timePart) return "";
    ...
    daySessions.forEach((session) => {
      ...
      const startedAt = formatTimeFromDatetime(session.started_at);
      const endedAt = formatTimeFromDatetime(session.ended_at);

      html += `<div class="session-card ${statusClass}">
          <div class="session-info">
              <h4>${session.title}${statusText}</h4>
              <div class="session-details">
                  <div>🕒 Scheduled: ${session.session_time} (${session.duration} min)</div>
                  ${startedAt ? `<div>✅ Started At: ${startedAt}</div>` : ""}
                  ${endedAt ? `<div>🏁 Ended At: ${endedAt}</div>` : ""}
                  <div>👤 ${session.client_name}</div>
                  <div>📋 ${session.session_type}</div>
                  ${session.description ? `<div>📝 ${session.description}</div>` : ""}
              </div>
...
getSessionStatusClass(session) {
  ...
  if (this.isSessionTimeCompleted(session)) {
    return "session-missed"  // changed class name
  }
...
/////////////////////////////////////////////////////////////////
  ...
  if (this.isSessionTimeCompleted(session)) {
    return "session-card-missed"  // changed from time-completed
  }
...
if (sessions.length === 0) {
  ...
  // ✅ Validate: startDate must be today or future
  const today = new Date()
  today.setHours(0, 0, 0, 0) // Normalize to midnight

  if (startDate < today) {
    alert("Start date is expired. Please select a valid future date.")
    return []
  }
...
sessionsByDate[date].forEach((session) => {
  ...
  if (!sessionStatus && this.isSessionTimeCompleted(session)) {
    sessionStatus = "missed"
  }

  const statusClass = this.getStatusClass(sessionStatus)
  const statusText = this.getStatusText(sessionStatus)

  html += `
    <div class="upcoming-session-card ${statusClass}" style="margin-bottom: 12px;">
      <div class="session-header">
        <h4>${session.title}</h4>
        <span class="status-badge status-${sessionStatus}">${statusText}</span>
      </div>
      <div class="session-details">
        <div>🕒 ${session.session_time} (${session.duration} min)</div>
        <div>👤 ${session.client_name}</div>
...
//   getSessionActionButtons(session) {
//   const now = new Date()
//   const sessionStart = new Date(`${session.session_date}T${session.session_time}`)

//   // If the session time is completed
//   if (this.isSessionTimeCompleted(session)) {
//     return '<span class="completed-badge">Time Completed</span>'
//   }
...
//     return `<button class="end-session-btn" onclick="dashboard.endSession(${session.id})">🔲 End Session</button>`
//   } else {
//     const isBeforeStart = now < sessionStart
//     return `<button 
//               class="start-session-btn" 
//               onclick="dashboard.startSession(${session.id})"
//               ${isBeforeStart ? 'disabled style="opacity: 0.5; cursor: not-allowed;" title="You can only start this session at the scheduled time."' : ''}
//             >
//               ▶️ Start Session
//             </button>`
//   }
// }
  ...
  try {
    const response = await fetch("sessions.php", {
      ...
      body: JSON.stringify({ id: sessionId, status: "started", actual_start_time: now })
    })
    const data = await response.json()
    if (data.success) {
      const sessionIndex = this.sessions.findIndex((s) => s.id == sessionId)
      if (sessionIndex !== -1) {
        this.sessions[sessionIndex].status = "started"
        this.sessions[sessionIndex].actual_start_time = now  // ✅ store actual time
      }
      this.openUpcomingModal()
      this.updateCalendarView()
    } else {
  ...
  try {
    const response = await fetch("sessions.php", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: sessionId, status: "completed", actual_end_time: now })
    })
    const data = await response.json()
    if (data.success) {
      const sessionIndex = this.sessions.findIndex((s) => s.id == sessionId)
      if (sessionIndex !== -1) {
        this.sessions[sessionIndex].status = "completed"
        this.sessions[sessionIndex].actual_end_time = now  // ✅ store actual time
      }
      this.openUpcomingModal()
      this.updateCalendarView()
    } else {
